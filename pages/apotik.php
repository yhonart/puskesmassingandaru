...
<?php
    ...
    $today = date("Y-m-d");
    $anamnesa = $con->prepare("SELECT DATE_FORMAT(created_date, '%Y-%m-%d') AS datecreate, `id_proses`,`nomor`,`id_pasien`,`tujuan` FROM `pasienproses` WHERE `status`='4'  AND DATE_FORMAT(created_date, '%Y-%m-%d')='$today'");
    $anamnesa->execute();
    $rowpas = $anamnesa->fetchAll();
?>
<div class="row">        
    <div class="col-lg-4">
        <h3>Daftar Antrian <?=$hakakses?></h3>        
